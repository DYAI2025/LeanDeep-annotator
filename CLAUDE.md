...
tools/                  # Python pipeline scripts (normalize, enrich, eval)
eval/                   # Gold corpus (99K msgs, 1543 chunks) + eval stats
tests/                  # Pytest suite (72 tests)
docs/
  ROADMAP.md            # 10 prioritized initiatives with full specs (P0-P3)
  BUGS.md               # 13 known bugs by severity
...
| SEM Detection | **Broken** | Only 27/238 fire (96.6% patternless) -> ROADMAP P0-1 |
| CLU Detection | **Nearly dead** | 30 detections on 99K msgs, 40.5% broken refs -> P0-2 |
| MEMA Detection | MVP | Keyword-based heuristic, no stateful tracking |
...
**P0 — Critical (SEM/CLU repair):**
1. **P0-1: SEM-Layer Reanimation** — give patterns to 211 dead SEMs, target >=120 firing
2. **P0-2: CLU Reference Repair** — fix 49/121 broken composed_of refs, target >=50 CLUs firing
3. **P0-3: Dead Marker Cleanup** — reclassify 7 UNKNOWN, triage 92 zero-detection markers
